# ============================================================
# 72_ml_benchmark_v7.R —— v7 主分析 ML 基准（GSE58485-only，11 基因）
# 由 19_d3_ml_benchmark.R 改造：输入 11 候选、24 样本；无 ComBat 敏感性
# （单队列无批次校正）；全部结果仅作探索性排序（补充材料）。
...
OUT <- file.path(ROOT, "results", "07_ML_Signature", "D3_ml_benchmark_v7")
...
# ---------- 1) 数据 ----------
cand <- toupper(read.csv(file.path(ROOT, "results", "16_Orthology",
                                   "v7_single_cohort",
...
clean_samples <- comp$sample[comp$include_clean & comp$cohort == "GSE58485"]
grp_map <- setNames(
...
stopifnot(ncol(expr_all) == sum(comp$include_clean & comp$cohort == "GSE58485"),
