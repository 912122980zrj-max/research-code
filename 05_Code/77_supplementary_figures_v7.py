...
ax.set_title("Eleven-algorithm benchmark, 11 candidates, n=24 (v7; descriptive only)")
...
fig.tight_layout()
fig.savefig(os.path.join(OUT, "S1a_algorithm_benchmark_v7.pdf"))
plt.close(fig)
...
for ax, cohort in zip(axes, cohorts):
    ...
    for xi, yi, g in zip(x, y, genes):
        ax.annotate(g, (xi, yi), textcoords="offset points", xytext=(5, 4),
                    fontsize=7.5)
    ...
    ax.grid(alpha=0.25)
fig.suptitle("Per-gene direction replication of the 11 candidate genes (v7)",
             fontsize=12)
fig.tight_layout(rect=(0, 0, 1, 0.94))
fig.savefig(os.path.join(OUT, "S2_direction_replication_v7.pdf"))
plt.close(fig)

print("wrote S1a_algorithm_benchmark_v7.pdf and S2_direction_replication_v7.pdf")
