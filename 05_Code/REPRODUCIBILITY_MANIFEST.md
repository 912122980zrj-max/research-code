...
the pre-v7 and stress-test pipeline (merged-cohort analyses are retained as
stress tests); scripts 70–78 are the v7 primary and submission pipeline:

- `70_single_cohort_main_v7.R` — GSE58485-only primary analysis (limma,
  WGCNA, candidate intersection; seed-independent except WGCNA soft-threshold
  selection).
- `71_permutation_v7.R` — measurability-matched permutation null
  (seed=2026, 10,000 draws).
- `72_ml_benchmark_v7.R` — descriptive 11-algorithm benchmark (seed=2026,
  5 repeats × 5-fold CV; PROBAST+AI high risk; supplementary only).
- `73_validation_v7.R` — direction replication (GSE128543/GSE205958) and
  human ortholog coverage.
- `74_go_kegg_v7.R` — GO/KEGG enrichment for the 11 candidates.
- `75_figures_v7.R` — main-text figures (volcano, module–trait, permutation,
  sensitivity heatmap, GO bubble).
- `76_compound_control.py` — ChEMBL single-source pyrene/phenanthrene control
  (S12).
- `77_supplementary_figures_v7.py` — S1a algorithm benchmark and S2 direction
  replication figures.
...
- R/Python figures are exported as vector PDFs (`theme_pub.R` /
  `ggsave_pub`).
- Figure 1 workflow: `58_figure1_workflow_v5.R` renders
  `Figure1_workflow_v5.png/.pdf`; editable source `Figure1_workflow_v5.drawio`.
- v7 main figures: `75_figures_v7.R`; supplementary figures S1a/S2:
...
- Manuscript DOCX: `59_build_manuscripts_docx.py` (EN/ZH masters); the
  ...
  `submission/01_Manuscript/MANUSCRIPT_v7_BMC_EN.md` with pandoc.
- Submission assembly: `scripts/_assemble_submission.ps1`;
  supplementary workbook: `68_build_supplementary_v6.py` (v7 sheets included).
