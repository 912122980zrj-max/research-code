...
p_qc1 <- plot_vln("nFeature", "nFeature_RNA", "A  snRNA-seq QC: nFeature by sample")
p_qc2 <- plot_vln("nCount", "nCount_RNA", "B  snRNA-seq QC: nCount by sample")
p_qc3 <- plot_vln("percent_mt", "Mitochondrial fraction (%)",
                  "C  snRNA-seq QC: mitochondrial fraction by sample")
ggsave_pub(file.path(ROOT, "results/11_SingleCell/figS5_qc_violin.pdf"),
           p_qc1 / p_qc2 / p_qc3 + plot_layout(heights = c(1, 1, 1)),
...
if (length(top_feat) > 0) {
  ...
  ggsave_pub(file.path(FIG_S, "figS5_marker_dotplot.pdf"),
             p_dot, width = 9, height = 4.5, dpi = 300)
